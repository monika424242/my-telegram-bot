...
def handle_plan(call):
    ...
    try:
        bot.send_photo(call.message.chat.id, QR_IMAGE_URL, caption=msg_text, parse_mode="Markdown")
    except Exception:
...
def handle_photo(message):
    ...
    admin_msg = (
        ...
        f"`/send {user.id} YOUR_COURSE_LINK`"
    )
...
def send_course_link(message):
    ...
    try:
        ...
        course_link = text_parts[2]
        customer_msg = f"🎉 **Payment Confirmed!**\n\nAapki videos ka link yeh raha:\n👉 {course_link}\n\nHumare sath judne ke liye dhanyawad!"
        bot.send_message(target_user_id, customer_msg, parse_mode="Markdown")
        bot.reply_to(message, f"✅ Success! User (`{target_user_id}`) ko link bhej diya gaya hai.")
    except Exception:
        bot.reply_to(message, "❌ **Format:** `/send User_ID Link`")
